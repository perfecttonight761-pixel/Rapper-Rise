    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="bg-[#121212] border border-white/10 rounded-2xl w-full max-w-2xl relative z-10 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto space-y-8">
           ...
           {/* Master Ownership */}
           <section>
              <h3 className="text-xs font-black uppercase tracking-[0.2em] text-white/40 mb-4 pb-2 border-b border-white/10">Master Rights Ownership</h3>
              <div className="bg-white/5 border border-white/10 p-6 rounded-3xl flex flex-col md:flex-row gap-6 md:items-center">
                 ...
                 <div className="flex-1">
                    {release.masterOwner ? (() => {
                        ...
                        return (
                            <>
                               <div className="text-sm font-bold text-red-400 mb-1">Owned by {label?.name || release.masterOwner}</div>
                               <p className="text-xs text-white/60 mb-4">Because this was released while signed to {label?.name}, they own the master recordings. They take their cut from all revenue generated by this release.</p>
                               {!isCurrentlySigned && (
                                   <button 
                                      onClick={() => {
                                          if (gameState.stats.money < buybackPrice) return window.alert("Not enough money to buy back the masters!");
                                          if (window.confirm(`Are you sure you want to buy back the masters for $${buybackPrice.toLocaleString()}? You will receive 100% of future royalties.`)) {
                                             setGameState(prev => {
                                                 if (!prev) return prev;
                                                 return {
                                                     ...prev,
                                      ...
                                      className="px-6 py-3 bg-purple-600 hover:bg-purple-500 rounded-xl text-white font-bold text-sm transition-colors"
                                   >
                                       Buy Back Masters (${Math.floor(buybackPrice).toLocaleString()})
                                   </button>
                               )}
                               {isCurrentlySigned && (
                                   <div className="text-xs font-bold text-white/40 italic">You cannot buy back masters while your contract is still active.</div>
                               )}
                            </>
                        )
                    })() : (
                        <>
                           <div className="text-sm font-bold text-green-400 mb-1">100% Independent</div>
                           <p className="text-xs text-white/60">You completely own the masters to this release. You receive 100% of the royalties generated.</p>
                        </>
                    )}
                 </div>
              </div>
